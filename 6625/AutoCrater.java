...
{
/** Autonomous crater side code for 6625 robot, to use remove @Disabled from line 10.
Also may need to change OP Mode name to avoid any conflicts with the code on the phone**/
    ...
    public void runOpMode() {
        ...
        while (!(isStopRequested() || getRuntime() - stage5 > 1.4));
        stage6 = getRuntime();
        motorLeft.setPower(Reverse);
        motorRight.setPower(Forward);
        // Robot turns
        while (!(isStopRequested() || getRuntime() - stage6 > .35)); 
        stage7 = getRuntime();
        motorLeft.setPower(Reverse);
        motorRight.setPower(Reverse);
        // Robot reverses
        while (!(isStopRequested() || getRuntime() - stage7 > 2));
        stage8 = getRuntime();
